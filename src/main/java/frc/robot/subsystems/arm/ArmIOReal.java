...
public class ArmIOReal implements ArmIO {
  ...
  public void periodic() {
    ...
    // If the motor is above (Return of bangController == 0), make the motor turn backwards until
    // not 0
    if (bangController.calculate(getPositionRads()) == 0) {
      while (bangController.calculate(getPositionRads()) == 0) {
        armMotor.set(-1);
      }
      // If the motor is below (Return of bangController == 1), make the motor turn Forward until
      // not 1
    } else {
